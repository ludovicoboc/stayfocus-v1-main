...
CREATE TABLE IF NOT EXISTS painel_dia (
    ...
    atividade text NOT NULL CHECK (length(trim(atividade)) > 0 AND length(atividade) <= 500),
...
CREATE INDEX IF NOT EXISTS idx_painel_dia_horario ON painel_dia(user_id, horario);
CREATE INDEX IF NOT EXISTS idx_painel_dia_concluida ON painel_dia(user_id, concluida);
...
CREATE INDEX IF NOT EXISTS idx_prioridades_importante ON prioridades(user_id, importante);
CREATE INDEX IF NOT EXISTS idx_prioridades_concluida ON prioridades(user_id, concluida);
...
CREATE INDEX IF NOT EXISTS idx_medicamentos_horario ON medicamentos(user_id, horario);
CREATE INDEX IF NOT EXISTS idx_medicamentos_tomado ON medicamentos(user_id, tomado);
...
CREATE INDEX IF NOT EXISTS idx_sessoes_foco_ativa ON sessoes_foco(user_id, ativa);
...
COMMENT ON COLUMN painel_dia.atividade IS 'Descrição da atividade (máx 500 caracteres)';
...
COMMENT ON COLUMN prioridades.titulo IS 'Título da prioridade (máx 200 caracteres)';
