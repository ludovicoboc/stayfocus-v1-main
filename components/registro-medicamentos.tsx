...
import { PlusCircle, X, CheckCircle, Clock, Pill, Edit, Trash2 } from "lucide-react"
import { format } from "date-fns"
import type { NovoMedicamento, FrequenciaMedicamento } from "@/types/saude"

export function RegistroMedicamentos() {
  const { medicamentos, loadingMedicamentos, resumoMedicamentos, adicionarMedicamento, excluirMedicamento } = useSaude()

  const [modalAberto, setModalAberto] = useState(false)
  const [novoMedicamento, setNovoMedicamento] = useState<Partial<NovoMedicamento>>({
    nome: "",
    dosagem: "",
    frequencia: "Diária",
    horarios: [],
  ...
  const handleSubmit = async () => {
    ...
    const intervalo_horas =
      novoMedicamento.frequencia === "Diária"
        ? Number.parseInt((novoMedicamento.intervalo_horas as unknown as string) || "24")
        : 24
    ...
    await adicionarMedicamento({
      ...
      data_inicio: novoMedicamento.data_inicio || format(new Date(), "yyyy-MM-dd"),
  ...
  return (
    <div className="mb-8">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold text-white">Registro de Medicamentos</h2>
        ...
        <Card className="bg-slate-800 border-slate-700">
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-white">{resumoMedicamentos.total}</p>
            </div>
        ...
        <Card className="bg-slate-800 border-slate-700">
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-white">{resumoMedicamentos.tomadosHoje}</p>
        ...
        <Card className="bg-slate-800 border-slate-700">
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-white">{resumoMedicamentos.proximaDose || "N/A"}</p>
      ...
      <Card className="bg-slate-800 border-slate-700">
        <CardHeader>
          <CardTitle className="text-white text-lg">Seus Medicamentos</CardTitle>
        </CardHeader>
        <CardContent>
          {loadingMedicamentos ? (
            ...
            <div className="border border-dashed border-slate-700 rounded-lg p-8 text-center">
              ...
              <p className="text-slate-400 mb-2">Você ainda não tem medicamentos cadastrados.</p>
              <p className="text-slate-500 text-sm">Adicione seu primeiro medicamento clicando no botão acima.</p>
            </div>
          ) : (
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="ml-3">
                      ...
                      <p className="text-sm text-slate-400">
                        Frequência: {med.frequencia}, Intervalo: {med.intervalo_horas} horas
                      </p>
                    </div>
                  </div>
      ...
      <Dialog open={modalAberto} onOpenChange={setModalAberto}>
        <DialogContent className="bg-slate-800 border-slate-700 text-white max-w-md">
          ...
          <div className="space-y-4">
            <div>
              <label className="text-sm text-slate-400 mb-1 block">Nome do Medicamento</label>
              <Input
            ...
            <div>
              <label className="text-sm text-slate-400 mb-1 block">Dosagem</label>
              <Input
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setNovoMedicamento({ ...novoMedicamento, frequencia: value as FrequenciaMedicamento })
                }
              >
            ...
            {novoMedicamento.frequencia === "Diária" && (
              <div>
                <label className="text-sm text-slate-400 mb-1 block">
                  ...
                  <span className="text-xs text-slate-500 ml-1">(tempo mínimo recomendado entre uma dose e outra)</span>
                </label>
                <Select
                  value={novoMedicamento.intervalo_horas?.toString() || "4"}
                  onValueChange={(value) =>
                    setNovoMedicamento({ ...novoMedicamento, intervalo_horas: Number.parseInt(value) })
                  }
                >
                  <SelectTrigger className="bg-slate-900 border-slate-700 text-white">
            ...
            <div>
              <label className="text-sm text-slate-400 mb-1 block">Horários</label>
              <div className="flex space-x-2">
                <Input
                  type="time"
                  className="bg-slate-900 border-slate-700 text-white"
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {novoMedicamento.horarios?.map((horario, index) => (
                  <Badge key={index} className="bg-slate-700 hover:bg-slate-600 text-white">
                    {horario}
                    <button className="ml-1 hover:text-red-400" onClick={() => removerHorario(index)}>
                      <X className="w-3 h-3" />
                    </button>
                  </Badge>
                ))}
            ...
            <div>
              <label className="text-sm text-slate-400 mb-1 block">Data de início</label>
              <Input
                type="date"
                className="bg-slate-900 border-slate-700 text-white"
                value={novoMedicamento.data_inicio}
                onChange={(e) => setNovoMedicamento({ ...novoMedicamento, data_inicio: e.target.value })}
              />
            </div>

            <div>
              <label className="text-sm text-slate-400 mb-1 block">Observações (opcional)</label>
              <Textarea
