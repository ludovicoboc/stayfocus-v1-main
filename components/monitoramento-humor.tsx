...
export function MonitoramentoHumor({ date }: { date?: string }) {
  ...
  useEffect(() => {
    setNovoRegistro((prev) => ({ ...prev, data: currentDate }));
  }, [currentDate]);
  ...
  const handleSubmit = async () => {
    ...
    await adicionarRegistroHumor({
      data: novoRegistro.data || currentDate,
  ...
  const registrosPorDia = diasDoMes.map((dia) => {
    const registro = registrosHumor.find((r) =>
      isSameDay(parseISO(r.data), dia),
    );
    return {
      dia,
      registro,
          ...
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-white">
                {resumoHumor.media.toFixed(1)}
              </p>
              <p className="text-xs text-slate-500">
                Baseado em todos os registros
              </p>
            </div>
        ...
        <CardContent>
          <div className="grid grid-cols-7 gap-1">
            ...
            {registrosPorDia.map(({ dia, registro }) => {
              ...
              const corClasse = nivelHumor
                ? cores[nivelHumor as keyof typeof cores]
                : "";
          ...
          {loadingRegistrosHumor ? (
            ...
            <div className="border border-dashed border-slate-700 rounded-lg p-8 text-center">
              ...
              <p className="text-slate-500 text-sm">
                Adicione seu primeiro registro de humor usando o botão "Novo
                Registro".
              </p>
            </div>
          ) : (
                  ...
                  >
                    <div className="flex items-center">
                      <div className="flex space-x-1">
                        {Array.from({ length: registro.nivel_humor }).map(
                          (_, i) => (
                            <div
                              key={i}
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex space-x-2 mt-2">
                {[1, 2, 3, 4, 5].map((nivel) =>
                  renderizarNivelHumor(
                    nivel,
                    nivel === novoRegistro.nivel_humor,
                    () =>
            ...
            <div>
              ...
              <Textarea
                ...
                onChange={(e) =>
                  setNovoRegistro({ ...novoRegistro, notas: e.target.value })
                }
              />
            </div>
          </div>
