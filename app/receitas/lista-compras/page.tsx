...
function ListaComprasContent() {
  ...
  const handleAdicionarItem = async () => {
    ...
    setAdicionando(true);
    try {
      await adicionarItemListaCompras(
        novoItem.nome,
        novoItem.categoria,
  ...
  const progresso =
    listaCompras.length > 0
      ? Math.round((itensComprados.length / listaCompras.length) * 100)
      : 0;

  const categorias = Array.from(
    new Set(listaCompras.map((item) => item.categoria)),
  ).sort();
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {Object.keys(itensAgrupadosNaoComprados).length > 0 ? (
            <div className="space-y-4">
              {Object.entries(itensAgrupadosNaoComprados).map(
                ([categoria, itens]) => (
                  <Card
                    key={categoria}
          ...
          {Object.keys(itensAgrupadosComprados).length > 0 && (
            <>
              ...
              <div className="space-y-4">
                ...
                {Object.entries(itensAgrupadosComprados).map(
                  ([categoria, itens]) => (
                    <Card
                      key={categoria}
                          ...
                          {itens.map((item) => (
                            ...
                            >
                              ...
                              <div className="flex-1">
                                ...
                                {item.quantidade && (
                                  <span className="text-green-500 text-sm ml-2 line-through">
                                    ({item.quantidade})
                                  </span>
                                )}
                              </div>
              ...
              <CardContent>
                <div className="space-y-2">
                  {categorias.map((categoria) => {
                    const itensCategoria = listaCompras.filter(
                      (item) => item.categoria === categoria,
                    );
                    const compradosCategoria = itensCategoria.filter(
                      (item) => item.comprado,
                    );
                    return (
                      <div
