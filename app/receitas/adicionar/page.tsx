...
export default function AdicionarReceitaPage() {
  ...
  const atualizarIngrediente = (index: number, valor: string) => {
    setReceita((prev) => ({
      ...prev,
      ingredientes: prev.ingredientes.map((ing, i) => (i === index ? valor : ing)),
    }))
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const ingredientesFiltrados = receita.ingredientes.filter((ing) => ing.trim() !== "")
    ...
    try {
      const { error } = await adicionarReceita({
        ...
        tempo_preparo: receita.tempo_preparo ? Number.parseInt(receita.tempo_preparo) : undefined,
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              <Card className="bg-slate-800 border-slate-700">
                ...
                <CardContent className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-2">Nome da Receita *</label>
                    <Input
                      value={receita.nome}
                      onChange={(e) => setReceita((prev) => ({ ...prev, nome: e.target.value }))}
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setReceita((prev) => ({ ...prev, categoria: value }))}
                    >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-slate-300 mb-2">Tempo de Preparo (min)</label>
                      <Input
                        type="number"
                        value={receita.tempo_preparo}
                        onChange={(e) => setReceita((prev) => ({ ...prev, tempo_preparo: e.target.value }))}
                        placeholder="30"
                        className="bg-slate-900 border-slate-600 text-white"
                      />
                    </div>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-slate-300 mb-2">Porções</label>
                      <Input
                        type="number"
                        value={receita.porcoes}
                        onChange={(e) => setReceita((prev) => ({ ...prev, porcoes: e.target.value }))}
                        placeholder="4"
                        className="bg-slate-900 border-slate-600 text-white"
                      />
                    </div>
              ...
              <Card className="bg-slate-800 border-slate-700">
                ...
                <CardContent>
                  <Textarea
                    ...
                    onChange={(e) => setReceita((prev) => ({ ...prev, modo_preparo: e.target.value }))}
            ...
            <div className="space-y-6">
              <Card className="bg-slate-800 border-slate-700">
                ...
                <CardContent className="space-y-4">
                  <div>
                    <h3 className="text-lg font-semibold text-white">{receita.nome || "Nome da Receita"}</h3>
                    {receita.categoria && (
                  ...
                  <div className="space-y-2 text-sm">
                    {receita.tempo_preparo && <div className="text-slate-300">⏱️ {receita.tempo_preparo} minutos</div>}
                    {receita.porcoes && <div className="text-slate-300">👥 {receita.porcoes} porções</div>}
                    <div className="flex items-center gap-2">
                      <span className="text-slate-300">📊</span>
                      <Badge className={`${getDificuldadeColor(receita.dificuldade)} text-white`}>
                        {receita.dificuldade}
                      </Badge>
                    </div>
                  </div>

                  {receita.ingredientes.filter((ing) => ing.trim()).length > 0 && (
                    <div>
                      ...
                      <ul className="text-xs text-slate-400 space-y-1">
                        ...
                        {receita.ingredientes.filter((ing) => ing.trim()).length > 3 && (
                          <li className="text-slate-500">
                            ... e mais {receita.ingredientes.filter((ing) => ing.trim()).length - 3}
                          </li>
                        )}
                      </ul>
                    </div>
