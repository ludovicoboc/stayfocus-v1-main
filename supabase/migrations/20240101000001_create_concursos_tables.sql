...
COMMENT ON COLUMN competition_simulations.description IS 'Description of the simulation content and purpose';
COMMENT ON COLUMN competition_simulations.questions IS 'JSON array of question IDs included in the simulation';
COMMENT ON COLUMN competition_simulations.question_count IS 'Total number of questions in the simulation';
COMMENT ON COLUMN competition_simulations.time_limit_minutes IS 'Time limit for completing the entire simulation';
COMMENT ON COLUMN competition_simulations.difficulty_filter IS 'Filter simulations by difficulty level';
COMMENT ON COLUMN competition_simulations.subject_filters IS 'Array of subjects to filter questions from';
COMMENT ON COLUMN competition_simulations.topic_filters IS 'Array of topics to filter questions from';
COMMENT ON COLUMN competition_simulations.results IS 'JSON object containing simulation results (score, answers, etc.)';
COMMENT ON COLUMN competition_simulations.status IS 'Current status of the simulation';
COMMENT ON COLUMN competition_simulations.is_favorite IS 'Whether the simulation is marked as favorite';
COMMENT ON COLUMN competition_simulations.is_public IS 'Whether the simulation can be shared with other users';
COMMENT ON COLUMN competition_simulations.attempts_count IS 'Number of times the simulation has been attempted';
COMMENT ON COLUMN competition_simulations.best_score IS 'Best score achieved in this simulation';
COMMENT ON COLUMN competition_simulations.avg_score IS 'Average score across all attempts';
...
CREATE INDEX IF NOT EXISTS idx_competition_simulations_is_public ON competition_simulations(is_public);
CREATE INDEX IF NOT EXISTS idx_competition_simulations_difficulty_filter ON competition_simulations(difficulty_filter);
CREATE INDEX IF NOT EXISTS idx_competition_simulations_subject_filters ON competition_simulations USING GIN(subject_filters);
CREATE INDEX IF NOT EXISTS idx_competition_simulations_topic_filters ON competition_simulations USING GIN(topic_filters);
CREATE INDEX IF NOT EXISTS idx_competition_simulations_best_score ON competition_simulations(best_score DESC);
