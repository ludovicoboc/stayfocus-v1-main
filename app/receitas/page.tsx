...
import { ChefHat, Plus, Search, ShoppingCart, Clock, Users, Star } from "lucide-react"
...
export default function ReceitasPage() {
  ...
  const getReceitasFiltradas = () => {
    return receitas.filter(receita => {
      ...
      const matchesCategory = categoriaFiltro === "todas" || receita.categoria === categoriaFiltro
  ...
  const toggleReceitaSelecionada = (id: string) => {
    setReceitasSelecionadas((prev) => (prev.includes(id) ? prev.filter((r) => r !== id) : [...prev, id]))
  }
    ...
    <div className="max-w-7xl mx-auto p-4">
        ...
        {receitasFiltradas.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {receitasFiltradas.map((receita) => (
              <Card key={receita.id} className="bg-slate-800 border-slate-700 hover:border-slate-600 transition-colors">
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <CardTitle className="text-white text-lg">{receita.nome}</CardTitle>
                    {receita.favorita && <Star className="w-5 h-5 text-yellow-500 fill-current" />}
                  </div>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <h4 className="text-sm font-medium text-slate-300">Ingredientes:</h4>
                    <div className="text-sm text-slate-400 max-h-20 overflow-y-auto">
                      ...
                      {receita.ingredientes.length > 3 && (
                        <div className="text-slate-500">... e mais {receita.ingredientes.length - 3}</div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      variant={receitasSelecionadas.includes(receita.id) ? "default" : "outline"}
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-white mb-2">
              {searchTerm || categoriaFiltro ? "Nenhuma receita encontrada" : "Nenhuma receita cadastrada"}
            </h3>
        ...
        {receitasSelecionadas.length > 0 && (
          <div className="fixed bottom-6 right-6">
            <Link href={`/receitas/lista-compras?receitas=${receitasSelecionadas.join(",")}`}>
